...
;(function($){
  ...
  /**
   * Generate buttons markup from the given _options_.
   *
  ...
  $.extend($.fn.tipswift, {
    effects: {
      ...
      show: function(tip, options) {
        ...
        if (options.actualGravity.indexOf('e') > -1) {
            start = '+=15';
            end = '-=15';
        }
        tip.css({visibility: 'visible', opacity: 0, left: start})
          .animate({left: end, opacity: options.opacity});
      },
      ...
      hide: function(tip, options) {
        tip.animate({
          left: options.actualGravity.indexOf('e') > -1?'-=15':'+=15',
          opacity: 0
        }, 
