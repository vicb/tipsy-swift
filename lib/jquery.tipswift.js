...
(function($) {
  ...
  $.extend(Tipswift.prototype, {
    show: function(content, options) {
      ...
      if (!this.shown && this.enabled) {
        ...
        $('.tipswift-inner', ov).remove();
        $('.tipswift-arrow', ov).after(content.addClass('tipswift-inner'));
        ov[0].className = 'tipswift';
        ov.css({top: 0, left: 0, visibility: 'hidden', display: 'block'}).appendTo(document.body);
        this.setPosition(o);
        ov.addClass('tipswift-' + this.gravity).addClass(o.extraClass.join(' '));
        o.showEffect.call(ov, o, this.gravity);
        this.shown = true;
      }
    },
    ...
    hide: function(options) {
      ...
      var o = options || this.options;
      if (this.shown) {
        o.hideEffect.call(this.getOverlay(), o, this.gravity);
        this.shown = false;
      }
    },
  ...
  $.tipSwift = {
    get: function(ele, options) {
      ...
      if (!ts) {
        ts = new Tipswift(ele, $.tipSwift.elementOptions(ele, options));
  ...
  $.extend($.tipSwift.plugins.tip.prototype, {
    ...
    show: function(e) { 
      var ts = $.tipSwift.get(e.target, this.options);
    ...
    doShow: function(ts) {
      ...
      o.filterContent(content, ts.getPositionner(o), ts);
      ts.show(content, o);
    },
  ...
  $.tipSwift.templates.dialog = function(btnTemplate, o, ts) {
    var parent = $('<div>'),
      title = $('<span class="tipswift-title"></span>'),
    ...
    o.filterContent(title, body, buttons, ts.getPositionner(o), ts);
    return parent.append(buttons).prepend(title.html(o.title));
  };
  ...
  $.extend($.tipSwift.plugins.dialog.prototype, {
    ...
    init: function(els, options) {
      ...
      o.html = true;
      o.extraClass.push('dialog');

      els = $(els);
      els[binder](o.showOn.join(' '), $.proxy(this.trigger, this));
      els[binder]('onBeforeShow', $.proxy(this.onBeforeShow, this));
      els[binder]('onBeforeHide', $.proxy(this.onBeforeHide, this));
    },
    ...
    trigger: function(e) {
      e.preventDefault();
      this.show($.tipSwift.get(e.target, this.options));
    },
    ...
    show: function(ts) {
      ...
      var o = this.options;
      ts.show(o.dialogTemplate(o.buttonTemplate, o, ts), o);
      ts._dialog.shown = true;
      openedDialog = ts;

      var ov = ts.getOverlay(), self = this;
    ...
    onBeforeShow: function(e) { return !$.tipSwift.get(e.target, this.options)._dialog.shown; },

    onBeforeHide: function(e) { return !$.tipSwift.get(e.target, this.options)._dialog.shown || hidingDialog; }
  });
